...
const addCall = async (req, res) => {
    try {
        ...
        const audits = await Audits.create(info);
...
const getCallById = async (req, res) => {
    try {
        const audits = await Audits.findOne({ where: { id: req.params.id } });
        return res.status(200).json({
            status: "ok",
            data: audits
        });
...
//     try {
//         const audits = await Audits.findAll({ where: { category: req.params.CategoryName} });
//         return res.status(200).json({
//             status: "ok",
//             data: audits
//         });
...
const updateCall = async (req, res) => {
    try {
        await Audits.update({ ...req.body }, { where: { id: req.params.id } });
...
const deleteCallById = async (req, res) => {
    try {
        await Audits.destroy({ where: { id: req.params.id } });
        return res.status(200).json({
            status: "ok",
